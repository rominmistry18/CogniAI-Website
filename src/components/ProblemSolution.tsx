...
import { AlertTriangle, CheckCircle2, ArrowRight, Users, Clock, TrendingDown, Brain, Target, Zap } from "lucide-react";
...
export function ProblemSolution({ 
  ...
  // Merge with defaults
  const problemsHeaderData = { ...defaultProblemsHeader, ...problemsHeader };
  const problemsData = problems && problems.length > 0 ? problems : defaultProblems;
  const solutionsHeaderData = { ...defaultSolutionsHeader, ...solutionsHeader };
  const solutionsData = solutions && solutions.length > 0 ? solutions : defaultSolutions;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-1 rounded-full text-sm font-medium bg-primary/10 text-primary mb-4">
            {problemsHeaderData.badge}
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4">
            {problemsHeaderData.title}
            <span className="gradient-text block">{problemsHeaderData.titleHighlight}</span>
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            {problemsHeaderData.subtitle}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {problemsData.map((problem, index) => {
            const IconComponent = iconMap[problem.icon] || Users;
            return (
        ...
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-1 rounded-full text-sm font-medium bg-accent/10 text-accent mb-4">
            {solutionsHeaderData.badge}
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4">
            {solutionsHeaderData.title}
            <span className="gradient-text block">{solutionsHeaderData.titleHighlight}</span>
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            {solutionsHeaderData.subtitle}
          </p>
        </div>
